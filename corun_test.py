...
def init_dist(args):
    ...
    opts.config.cta_policy = 2 #dist.ProcessGroupNCCL.NCCL_CTA_POLICY_ZERO
...
def bandwidth_test(rank, size, world_size, group_name, comm):
    ...
    # Allocate tensors using symmetric memory (required for CE collectives)
    inp = symm_mem.empty(num_elems, device=device)
    out = symm_mem.empty(num_elems * world_size, device=device)
    ...
    if comm == "all_gather":
        print("Doing all_gather (CE)")
        for i in range(1):
            work = dist.all_gather_into_tensor(out, inp, async_op=True)
            work.wait()
    else:
        print("Doing reduce_scatter (CE)")
        for i in range(1):
            work = dist.reduce_scatter_tensor(
                inp, out, torch.distributed.ReduceOp.AVG, async_op=True
            )
            work.wait()
